...
// Added by Tim to remove compile error on MacOS
...
Tree* RandomForest::get_tree(unsigned int index) const {
...
void RandomForest::add_tree(Tree* tree) {
...
void RandomForest::compute_skewed_proximity(const InstanceSet& set,
  ...
  for (unsigned int i = 0; i < prox->size(); ++i) {
    for ( unsigned int j = 0; j < prox->size(); ++j) {
...
void RandomForest::variable_importance(vector< pair< float, int> >*ranking,
  ...
  for (unsigned int i = 0; i < trees_.size(); ++i) {
    ...
    //aggregate
    for (unsigned int j = 0; j < tree_importance.size(); ++j) {
  ...
  // Write the z-scores
  for (unsigned int i = 0; i < raw_scores.size(); ++i) {
