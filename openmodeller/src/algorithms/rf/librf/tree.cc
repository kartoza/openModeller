...
void Tree::build_tree(int min_size) {
  ...
  do {
    ...
    built_nodes++;
  } while ((unsigned int)built_nodes < nodes_.size());
}
...
void Tree::add_node(tree_node n) {
  nodes_.push_back(n);
}
...
tree_node* Tree::get_node(unsigned int index) const {

  if (index >= nodes_.size()) {
    return NULL;
  }

  return const_cast<tree_node*>(&nodes_[index]);
}
...
void Tree::variable_importance(vector<float>* score,
  ...
  for (unsigned int i = 0; i < set_.num_attributes(); ++i) {
