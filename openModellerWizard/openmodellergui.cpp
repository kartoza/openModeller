...
void OpenModellerGui::formSelected(const QString &thePageNameQString)
{
  ...
  if (thePageNameQString==tr("Step 1 of 9")) //we do this when arriving at the mode selection page
  {
    ...
    {
      bool myAlgExists = false;

      //check if last algorithm still exists 
      for (int i=0; i <= cboModelAlgorithm->count(); i++)
      {
        cboModelAlgorithm->setCurrentItem(i);
        if (cboModelAlgorithm->currentText().compare(myModelName) == 0)
        {
          //if exists, set it as the default
      ...
      //if last algorithm does not exist anymore
      if (!myAlgExists) {
        // select first algorithm in list
        cboModelAlgorithm->setCurrentItem(0);
        myModelName = cboModelAlgorithm->currentText();
  ...
  else if (thePageNameQString==tr("Step 2 of 9")) 
  {
  ...
  else if (thePageNameQString==tr("Step 3 of 9")) 
  {
    setNextEnabled(currentPage(),false);
  ...
  else if (thePageNameQString==tr("Step 4 of 9")) 
  {  
  ...
  else if (thePageNameQString==tr("Step 5 of 9"))
  {
  ...
  else if (thePageNameQString==tr("Step 6 of 9"))
  {
    ...
    if (!myInputMask.isEmpty())
    {
      ...
      //the preferred one - note that setDupliactesAllowed is only applicable to 
      //editable combo boxes
      for (i=0; i <= cboInputMaskLayer->count(); i++)
      {
        cboInputMaskLayer->setCurrentItem(i);

        if (cboInputMaskLayer->currentText().compare(myInputMask) == 0)
        {
          myFlag=true;
          break;
        }
    ...
    if (!myOutputMask.isEmpty())
    {
      ...
      for (i=0; i <= cboInputMaskLayer->count(); i++)
      {
        ...
        if (cboOutputMaskLayer->currentText().compare(myOutputMask) == 0)
        {
          myFlag=true;
          break;
        }
    ...
    if (!myOutputFormat.isEmpty())
    {
      ...
      for ( i=0; i <= cboOutputFormatLayer->count(); i++)
      {
        ...
        if (cboOutputFormatLayer->currentText().compare(myOutputMask) == 0)
        {
          myFlag=true;
          break;
        }
  ...
  else if (thePageNameQString==tr("Step 7 of 9")) 
  {  
  ...
  else if (thePageNameQString==tr("Step 8 of 9")) 
  {  
    //
...
void OpenModellerGui::parseAndRun(QString theParametersFileNameQString)
{
  ...
  QString myUnderscoreTaxonNameQString = taxonNameQString;
  myUnderscoreTaxonNameQString.replace(" ","_");

  try 
  {
    txtbLogs->append( QString("Library version %1\n").arg( mOpenModeller->getVersion() ) );
    ...
    createModelImage(outputFileNameQString + "-" + myUnderscoreTaxonNameQString);
    // and a smaller version for this report
    QString mySmallImage = createResizedImage(outputFileNameQString + "-" + myUnderscoreTaxonNameQString);
    txtbLogs->append("<img src=\""+mySmallImage+"\">");

    //Create ascii grid file
    QString myAsciiFilename = OmgGdal::Gdal2Ascii(outputFileNameQString + "-" + myUnderscoreTaxonNameQString+".tif");
    ...
    emit drawRasterLayer(outputFileNameQString + QString("-") + myUnderscoreTaxonNameQString + QString(".tif"));
  ...
  QString myLogFileName = outputFileNameQString + "-" + myUnderscoreTaxonNameQString + ".html";
  QString myImageFileName = outputFileNameQString + "-" + myUnderscoreTaxonNameQString + ".png";
  //write teh logs to a file too
  QFile myQFile(myLogFileName);
  ...
  {
    //Problem creating log file
    std::cout << "Filename '" << myLogFileName.ascii() << "' is invalid" << std::endl;
    QMessageBox::warning( this, "Error creating log file!", QString("The filename and/or directory is invalid: %1").arg(myLogFileName.ascii()) );
  }
  emit modelDone(myImageFileName, myLogFileName, myUnderscoreTaxonNameQString);
...
{
  QString myUnderscoreName=taxonNameQString;
  myUnderscoreName.replace(" ","_");
  QFile myQFile( outputFileNameQString +"-" + myUnderscoreName + ".cfg");
  std::cout << "Config file name: " << outputFileNameQString.ascii() << std::endl;
  if ( myQFile.open( IO_WriteOnly ) ) {
    QTextStream myQTextStream( &myQFile );
    //write the header to the file
    ...
    myQTextStream << tr("Output model = ") << outputFileNameQString + "-" + myUnderscoreName << ".xml\n";

    myQTextStream << tr("\n\n##\n");
    myQTextStream << tr("# Output file name (should end in .tif)\n");
    myQTextStream << tr("Output file = ") << outputFileNameQString + "-" + myUnderscoreName << ".tif\n";
    myQTextStream << tr("\n\n##\n"); 
...
if (myLocalitiesType == "single")
{
	...
	for ( unsigned int mySpeciesInt = 0; mySpeciesInt < lstTaxa->count(); mySpeciesInt++ )
	{
		...
		if (lstTaxa->isSelected (mySpeciesInt))
		{
			...
			QString myUnderscoreName=taxonNameQString;
			myUnderscoreName.replace(" ","_");
			parseAndRun(outputFileNameQString +"-" + myUnderscoreName + ".cfg");
			QString myOutputThumbnail = outputFileNameQString +"-" + myUnderscoreName + ".png";
  ...
  if ( myQFile.open( IO_ReadOnly ) ) 
  {
    ...
    while ( !myQTextStream.atEnd() ) 
    {
      ...
      if (myTaxonQString != "")
      {
        ...
        myTaxonQString=myTaxonQString.replace( "\t", " " );
