...
OmgClimateDataProcessorController::OmgClimateDataProcessorController() : QObject()
{
    ...
    inputFileType=OmgClimateFileReader::GDAL;
...
bool OmgClimateDataProcessorController::makeInputFileTypeMap()
{
    QString myString;
    OmgClimateFileReader::FileType myFileType;
    inputFileTypeMap.clear();
    // Copied from OmgClimateFileReader header file:
    ...
    myFileType=OmgClimateFileReader::GDAL;
    ...
    myString=myString.toUpper();
    //set its associated enum
    myFileType=OmgClimateFileReader::HADLEY_SRES;
    //add it to the associative array
    inputFileTypeMap[myString]=myFileType;
    ...
    myString=myString.toUpper();
    //set its associated enum
    myFileType=OmgClimateFileReader::HADLEY_IS92;
    //add it to the associative array
    inputFileTypeMap[myString]=myFileType;
    ...
    myString=myString.toUpper();
    //set its associated enum
    myFileType=OmgClimateFileReader::IPCC_OBSERVED;
    //add it to the associative array
    inputFileTypeMap[myString]=myFileType;
    ...
    myString=myString.toUpper();
    //set its associated enum
    myFileType=OmgClimateFileReader::VALDES;
    //add it to the associative array
    inputFileTypeMap[myString]=myFileType;
    ...
    myString=myString.toUpper();
    //set its associated enum
    myFileType=OmgClimateFileReader::ECHAM4;
    //add it to the associative array
    inputFileTypeMap[myString]=myFileType;
    ...
    myString=myString.toUpper();
    //set its associated enum
    myFileType=OmgClimateFileReader::CSIRO_MK2;
    //add it to the associative array
    inputFileTypeMap[myString]=myFileType;
    ...
    myString=myString.toUpper();
    //set its associated enum
    myFileType=OmgClimateFileReader::NCAR_CSM_PCM;
    //add it to the associative array
    inputFileTypeMap[myString]=myFileType;
    ...
    myString=myString.toUpper();
    //set its associated enum
    myFileType=OmgClimateFileReader::GFDL_R30;
    //add it to the associative array
    inputFileTypeMap[myString]=myFileType;
    ...
    myString=myString.toUpper();
    //set its associated enum
    myFileType=OmgClimateFileReader::CGCM2;
    //add it to the associative array
    inputFileTypeMap[myString]=myFileType;
    ...
    myString=myString.toUpper();
    //set its associated enum
    myFileType=OmgClimateFileReader::CCSR_AGCM_OGCM;
    //add it to the associative array
    inputFileTypeMap[myString]=myFileType;
...
bool OmgClimateDataProcessorController::makeOutputFileTypeMap()
{
    QString myString;
    OmgFileWriter::FileType myFileType;
    outputFileTypeMap.clear();
    // Copied from OmgFileWriter header file:
    ...
    myString=myString.toUpper();
    //set its associated enum
    myFileType=OmgFileWriter::ESRI_ASCII;
    //add it to the associative array
    ...
    myString=myString.toUpper();
    //set its associated enum
    myFileType=OmgFileWriter::MATLAB;
    //add it to the associative array
    outputFileTypeMap[myString]=myFileType;
    ...
    myString=myString.toUpper();
    //set its associated enum
    myFileType=OmgFileWriter::PLAIN;
    //add it to the associative array
    outputFileTypeMap[myString]=myFileType;
    return true;
}
 ...
 *           Default is 1.
 */
OmgClimateFileGroup *OmgClimateDataProcessorController::initialiseFileGroup(QString theFileName,int theStartYear=1)
{
    OmgClimateFileGroup * myFileGroup = new OmgClimateFileGroup();
    ...
    if (filesInSeriesFlag)
    {
       ...
       QString myFileNameBase = myFileName.left(myFileName.length()-2);//e.g. 'somefile00' becomes 'somefile'
        ...
        for (int myInt=1; myInt < 13; myInt++)
        {
            ...
            OmgClimateFileReader *myOmgClimateFileReader = new OmgClimateFileReader();
            myOmgClimateFileReader->initialise(myCurrentFileName,inputFileType);            
            qDebug( "initialiseFileGroup - *** Adding " + myCurrentFileName.toLocal8Bit()
            + " to file group *********************" );
            myFileGroup->add(myOmgClimateFileReader);
    ...
    else
    {
        ...
        {
            OmgClimateFileReader *myOmgClimateFileReader = new OmgClimateFileReader();
            myOmgClimateFileReader->initialise(theFileName,inputFileType);            
            myOmgClimateFileReader->setActiveBlock(myInt);
            myFileGroup->add(myOmgClimateFileReader);
...
/** Read property of OmgClimateFileReader::FileType FileType. */
const OmgClimateFileReader::FileType OmgClimateDataProcessorController::getInputFileType()
{
    return inputFileType;
}
/** Write property of OmgClimateFileReader::FileType FileType. */
...
void OmgClimateDataProcessorController::setInputFileType( const QString theInputFileTypeString)
{
    ...
    OmgClimateFileReader::FileType myInputFileType;
...
/** Read property of OmgFileWriter::FileType outputFileType. */
const OmgFileWriter::FileType OmgClimateDataProcessorController::getOutputFileType()
{
    return outputFileType;
}
/** Write property of OmgFileWriter::FileType FileType. */
...
void OmgClimateDataProcessorController::setOutputFileType( const QString theOutputFileTypeString)
{
    ...
    OmgFileWriter::FileType myOutputFileType;
    //make sure the fileTypeMap exists
...
{
#ifdef QGISDEBUG
    qDebug( "OmgClimateDataProcessorController::makeAvailableCalculationsMap() called!" );
#endif
...
bool OmgClimateDataProcessorController::run()
{
  ...
  QMap<QString, OmgFileWriterStruct> myOmgFileWriterMap;

  for (myIter=availableCalculationsMap.begin(); myIter != availableCalculationsMap.end(); myIter++)
  {
    if (myIter.value()) //true
    {
      qDebug( "Adding " +  myIter.key().toLocal8Bit() + " to myOmgFileWriterMap\n");
      ...
      OmgFileWriter * myOmgFileWriter = new OmgFileWriter(myFileName,outputFileType);

      //Use externally defined header if its been set
      if (!outputHeader.isEmpty())
      {
        myOmgFileWriter->writeString(outputHeader);
      }
      ...
      qDebug( "Added " + myOmgFileWriter->fileName().toLocal8Bit() );
      OmgFileWriterStruct myOmgFileWriterStruct;
      myOmgFileWriterStruct.fileWriter=myOmgFileWriter;
      myOmgFileWriterStruct.fullFileName=myFileName;
      //add it to the map
      myOmgFileWriterMap[myIter.key()]=myOmgFileWriterStruct;
      qDebug( "Added " + myOmgFileWriterStruct.fullFileName.toLocal8Bit() );
    }
  }
          ...
          availableCalculationsMap["Mean temperature"])
  {
    ...
    qDebug( "OmgClimateDataProcessorController::run - Mean temperature requested" );
    ...
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Mean temperature"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    ...
    while (!meanTempFileGroup->isAtMatrixEnd())
    {
      ...
      bool myResultFlag = myOmgFileWriter->writeElement( myFloat );
      if (!myResultFlag)
      {
        qDebug( "Error! Writing an element to " +  myOmgFileWriterStruct.fullFileName.toLocal8Bit() +  " failed " );
      ...
      {
        myOmgFileWriter->writeString("\n");
      }
      ////emit cellDone( myFloat );
    }
    qDebug( " ++++++ Emitting variableDone signal! " );
    emit variableDone(myOmgFileWriterStruct.fullFileName);
          ...
          availableCalculationsMap["Annual mean diurnal temperature range"])
  {
    ...
    qDebug( "OmgClimateDataProcessorController::run Annual mean diurnal temperature range requested" );
    //move to start of the current data matrix
    diurnalTempFileGroup->rewind();
    //get the struct containing the filewriter pointer and full file name from the writer map
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Annual mean diurnal temperature range"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    int myXCountInt=0;
          ...
          availableCalculationsMap["Annual mean number of frost days"])
  {
    ...
    qDebug( "OmgClimateDataProcessorController::run Annual mean number of frost days requested" );
    //move to start of the current data matrix
    frostDaysFileGroup->rewind();
    //get the struct containing the filewriter pointer and full file name from the writer map
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Annual mean number of frost days"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    int myXCountInt=0;
          ...
          availableCalculationsMap["Annual mean total incident solar radiation"])
  {
    ...
    qDebug( "OmgClimateDataProcessorController::run Annual mean total incident solar radiation requested" );
    //move to start of the current data matrix
    totalSolarRadFileGroup->rewind();
    //get the struct containing the filewriter pointer and full file name from the writer map
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Annual mean total incident solar radiation"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    int myXCountInt=0;
          ...
          && availableCalculationsMap["Annual temperature range"])
  {
    ...
    qDebug( "OmgClimateDataProcessorController::run - Annual temperature range requested" );
    //move to start of the current data matrix
    minTempFileGroup->rewind();
    maxTempFileGroup->rewind();
    //get the struct containing the filewriter pointer and full file name from the writer map
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Annual temperature range"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    int myXCountInt=0;
          ...
          && availableCalculationsMap["Highest temperature in warmest month"])
  {
    ...
    qDebug( "OmgClimateDataProcessorController::run - Highest temperature in warmest month requested" );
    maxTempFileGroup->rewind();
    //get the struct containing the filewriter pointer and full file name from the writer map
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Highest temperature in warmest month"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    int myXCountInt=0;
          ...
          && availableCalculationsMap["Lowest temperature in coolest month"])
  {
    ...
    qDebug( "OmgClimateDataProcessorController::run - Lowest temperature in coolest month requested" );
    minTempFileGroup->rewind();
    //get the struct containing the filewriter pointer and full file name from the writer map
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Lowest temperature in coolest month"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    int myXCountInt=0;
    while (!minTempFileGroup->isAtMatrixEnd())
          ...
          && availableCalculationsMap["Mean daily precipitation"])
  {
    ...
    qDebug( "OmgClimateDataProcessorController::run Mean daily precipitation" );
    meanPrecipFileGroup->rewind();
    //get the struct containing the filewriter pointer and full file name from the writer map
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Mean daily precipitation"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    int myXCountInt=0;
          ...
          && availableCalculationsMap["Mean daily precipitation in coolest month"])
  {
    ...
    qDebug( "OmgClimateDataProcessorController::run Mean daily precipitation in coolest month" );
    meanPrecipFileGroup->rewind();
    meanTempFileGroup->rewind();
    //get the struct containing the filewriter pointer and full file name from the writer map
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Mean daily precipitation in coolest month"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    int myXCountInt=0;
    while (!meanPrecipFileGroup->isAtMatrixEnd())
          ...
          && availableCalculationsMap["Mean daily precipitation in coolest quarter"])
  {
    ...
    meanTempFileGroup->rewind();
    //get the struct containing the filewriter pointer and full file name from the writer map
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Mean daily precipitation in coolest quarter"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    int myXCountInt=0;
    while (!meanPrecipFileGroup->isAtMatrixEnd())
          ...
          availableCalculationsMap["Mean daily precipitation in driest month"])
  {
    ...
    qDebug( "OmgClimateDataProcessorController::run Mean daily precipitation in driest month" );
    meanPrecipFileGroup->rewind();
    //get the struct containing the filewriter pointer and full file name from the writer map
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Mean daily precipitation in driest month"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    int myXCountInt=0;
    while (!meanPrecipFileGroup->isAtMatrixEnd())
          ...
          availableCalculationsMap["Mean daily precipitation in driest quarter"])
  {
    ...
    qDebug( "OmgClimateDataProcessorController::run Mean daily precipitation in driest quarter" );
    meanPrecipFileGroup->rewind();
    //get the struct containing the filewriter pointer and full file name from the writer map
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Mean daily precipitation in driest quarter"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    int myXCountInt=0;
    while (!meanPrecipFileGroup->isAtMatrixEnd())
          ...
          && availableCalculationsMap["Mean daily precipitation in warmest month"])
  {
    ...
    qDebug( "OmgClimateDataProcessorController::run Mean daily precipitation in warmest month" );
    //move to the start of data blocks
    meanPrecipFileGroup->rewind();
    meanTempFileGroup->rewind();
    //get the struct containing the filewriter pointer and full file name from the writer map
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Mean daily precipitation in warmest month"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    int myXCountInt=0;
    while (!meanPrecipFileGroup->isAtMatrixEnd())
          ...
          &&availableCalculationsMap["Mean daily precipitation in warmest quarter"])
  {
    ...
    qDebug( "OmgClimateDataProcessorController::run Mean daily precipitation in warmest quarter" );
    meanPrecipFileGroup->rewind();
    meanTempFileGroup->rewind();
    //get the struct containing the filewriter pointer and full file name from the writer map
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Mean daily precipitation in warmest quarter"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    int myXCountInt=0;
    while (!meanPrecipFileGroup->isAtMatrixEnd())
          ...
          availableCalculationsMap["Mean daily precipitation in wettest month"])
  {
    ...
    qDebug( "OmgClimateDataProcessorController::run Mean daily precipitation in wettest month" );
    meanPrecipFileGroup->rewind();
    //get the struct containing the filewriter pointer and full file name from the writer map
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Mean daily precipitation in wettest month"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    int myXCountInt=0;
    while (!meanPrecipFileGroup->isAtMatrixEnd())
          ...
          availableCalculationsMap["Mean daily precipitation in wettest quarter"])
  {
    ...
    qDebug( "OmgClimateDataProcessorController::run Mean daily precipitation in wettest quarter" );
    meanPrecipFileGroup->rewind();
    //get the struct containing the filewriter pointer and full file name from the writer map
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Mean daily precipitation in wettest quarter"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    int myXCountInt=0;
    while (!meanPrecipFileGroup->isAtMatrixEnd())
          ...
          && availableCalculationsMap["Mean diurnal temperature range in coolest month"])
  {
    ...
    qDebug( "OmgClimateDataProcessorController::run Mean diurnal temperature range in coolest month" );
    diurnalTempFileGroup->rewind();
    meanTempFileGroup->rewind();
    //get the struct containing the filewriter pointer and full file name from the writer map
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Mean diurnal temperature range in coolest month"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    int myXCountInt=0;
    while (!diurnalTempFileGroup->isAtMatrixEnd())
          ...
          && availableCalculationsMap["Mean diurnal temperature range in warmest month"])
  {
    ...
    qDebug( "OmgClimateDataProcessorController::run Mean diurnal temperature range in warmest month" );
    diurnalTempFileGroup->rewind();
    meanTempFileGroup->rewind();
    //get the struct containing the filewriter pointer and full file name from the writer map
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Mean diurnal temperature range in warmest month"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    int myXCountInt=0;
    while (!diurnalTempFileGroup->isAtMatrixEnd())
          ...
          && availableCalculationsMap["Mean precipitation in frost free months"])
  {
    ...
    qDebug( "OmgClimateDataProcessorController::run Mean precipitation in frost free months" );
    meanPrecipFileGroup->rewind();
    frostDaysFileGroup->rewind();
    //get the struct containing the filewriter pointer and full file name from the writer map
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Mean precipitation in frost free months"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    int myXCountInt=0;
    while (!meanPrecipFileGroup->isAtMatrixEnd())
          ...
          availableCalculationsMap["Mean temperature in coolest month"])
  {
    ...
    qDebug( "OmgClimateDataProcessorController::run Mean temperature in coolest month" );

    meanTempFileGroup->rewind();
    //get the struct containing the filewriter pointer and full file name from the writer map
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Mean temperature in coolest month"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    int myXCountInt=0;
    while (!meanTempFileGroup->isAtMatrixEnd())
          ...
          availableCalculationsMap["Mean temperature in coolest quarter"])
  {
    ...
    qDebug( "OmgClimateDataProcessorController::run Mean temperature in coolest quarter" );
    meanTempFileGroup->rewind();
    //get the struct containing the filewriter pointer and full file name from the writer map
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Mean temperature in coolest quarter"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    int myXCountInt=0;
    while (!meanTempFileGroup->isAtMatrixEnd())
          ...
          && availableCalculationsMap["Mean temperature in frost free months"])
  {
    ...
    qDebug( "OmgClimateDataProcessorController::run Mean temperature in frost free months" );
    meanTempFileGroup->rewind();
    frostDaysFileGroup->rewind();
    //get the struct containing the filewriter pointer and full file name from the writer map
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Mean temperature in frost free months"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    int myXCountInt=0;
    while (!meanTempFileGroup->isAtMatrixEnd())
          ...
          availableCalculationsMap["Mean temperature in warmest month"])
  {
    ...
    qDebug( "OmgClimateDataProcessorController::run Mean temperature in warmest month" );
    meanTempFileGroup->rewind();
    //get the struct containing the filewriter pointer and full file name from the writer map
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Mean temperature in warmest month"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    int myXCountInt=0;
    while (!meanTempFileGroup->isAtMatrixEnd())
          ...
          availableCalculationsMap["Mean temperature in warmest quarter"])
  {
    ...
    qDebug( "OmgClimateDataProcessorController::run Mean temperature in warmest quarter" );
    meanTempFileGroup->rewind();
    //get the struct containing the filewriter pointer and full file name from the writer map
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Mean temperature in warmest quarter"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    int myXCountInt=0;
    while (!meanTempFileGroup->isAtMatrixEnd())
          ...
          availableCalculationsMap["Mean wind speed"])
  {
    ...
    qDebug( "OmgClimateDataProcessorController::run Mean wind speed" );
    windSpeedFileGroup->rewind();
    //get the struct containing the filewriter pointer and full file name from the writer map
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Mean wind speed"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    int myXCountInt=0;
          ...
          availableCalculationsMap["Number of months with minimum temperature above freezing"])
  {
    ...
    qDebug( "OmgClimateDataProcessorController::run Number of months with minimum temperature above freezing" );
    minTempFileGroup->rewind();
    //get the struct containing the filewriter pointer and full file name from the writer map
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Number of months with minimum temperature above freezing"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    int myXCountInt=0;
    while (!minTempFileGroup->isAtMatrixEnd())
          ...
          && availableCalculationsMap["Radiation in coolest month"])
  {
    ...
    qDebug( "OmgClimateDataProcessorController::run Radiation in coolest month" );
    totalSolarRadFileGroup->rewind();
    meanTempFileGroup->rewind();
    //get the struct containing the filewriter pointer and full file name from the writer map
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Radiation in coolest month"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    int myXCountInt=0;
    while (!totalSolarRadFileGroup->isAtMatrixEnd())
  ...
  {
    emit variableStart("Radiation in coolest quarter");
    qDebug( "OmgClimateDataProcessorController::run Radiation in coolest quarter" );
    totalSolarRadFileGroup->rewind();
    meanTempFileGroup->rewind();
    //get the struct containing the filewriter pointer and full file name from the writer map
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Radiation in coolest quarter"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    int myXCountInt=0;
    while (!totalSolarRadFileGroup->isAtMatrixEnd())
          ...
          && availableCalculationsMap["Radiation in warmest month"])
  {
    ...
    qDebug( "OmgClimateDataProcessorController::run Radiation in warmest month" );
    totalSolarRadFileGroup->rewind();
    meanTempFileGroup->rewind();
    //get the struct containing the filewriter pointer and full file name from the writer map
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Radiation in warmest month"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    int myXCountInt=0;
    while (!totalSolarRadFileGroup->isAtMatrixEnd())
          ...
          && availableCalculationsMap["Radiation in warmest quarter"])
  {
    ...
    qDebug( "OmgClimateDataProcessorController::run Radiation in warmest quarter" );
    totalSolarRadFileGroup->rewind();
    meanTempFileGroup->rewind();
    //get the struct containing the filewriter pointer and full file name from the writer map
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Radiation in warmest quarter"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    int myXCountInt=0;
    while (!totalSolarRadFileGroup->isAtMatrixEnd())
          ...
          && availableCalculationsMap["Radiation in driest month"])
  {
    ...
    qDebug( "OmgClimateDataProcessorController::run Radiation in driest month" );
    totalSolarRadFileGroup->rewind();
    meanPrecipFileGroup->rewind();
    //get the struct containing the filewriter pointer and full file name from the writer map
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Radiation in driest month"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    int myXCountInt=0;
    while (!totalSolarRadFileGroup->isAtMatrixEnd())
          ...
          && availableCalculationsMap["Radiation in driest quarter"])
  {
    ...
    qDebug( "OmgClimateDataProcessorController::run Radiation in driest quarter" );
    totalSolarRadFileGroup->rewind();
    meanPrecipFileGroup->rewind();
    //get the struct containing the filewriter pointer and full file name from the writer map
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Radiation in driest quarter"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    int myXCountInt=0;
    while (!totalSolarRadFileGroup->isAtMatrixEnd())
          ...
          && availableCalculationsMap["Radiation in wettest month"])
  {
    ...
    qDebug( "OmgClimateDataProcessorController::run Radiation in wettest month" );
    totalSolarRadFileGroup->rewind();
    meanPrecipFileGroup->rewind();
    //get the struct containing the filewriter pointer and full file name from the writer map
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Radiation in wettest month"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    int myXCountInt=0;
    while (!totalSolarRadFileGroup->isAtMatrixEnd())
          ...
          && availableCalculationsMap["Radiation in wettest quarter"])
  {
    ...
    qDebug( "OmgClimateDataProcessorController::run Radiation in wettest quarter" );
    totalSolarRadFileGroup->rewind();
    meanPrecipFileGroup->rewind();
    //get the struct containing the filewriter pointer and full file name from the writer map
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Radiation in wettest quarter"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    int myXCountInt=0;
    while (!totalSolarRadFileGroup->isAtMatrixEnd())
          ...
          availableCalculationsMap["Standard deviation of mean precipitation"])
  {
    ...
    qDebug( "OmgClimateDataProcessorController::run Standard deviation of mean precipitation" );
    meanPrecipFileGroup->rewind();
    //get the struct containing the filewriter pointer and full file name from the writer map
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Standard deviation of mean precipitation"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    int myXCountInt=0;
    while (!meanPrecipFileGroup->isAtMatrixEnd())
          ...
          && availableCalculationsMap["Standard deviation of mean temperature"])
  {
    ...
    qDebug( "OmgClimateDataProcessorController::run Standard deviation of mean temperature" );
    meanTempFileGroup->rewind();
    //get the struct containing the filewriter pointer and full file name from the writer map
    OmgFileWriterStruct myOmgFileWriterStruct = myOmgFileWriterMap["Standard deviation of mean temperature"];
    //get the filewriter from out of the struct
    OmgFileWriter *myOmgFileWriter = myOmgFileWriterStruct.fileWriter;
    int myXCountInt=0;
    while (!meanTempFileGroup->isAtMatrixEnd())
...
void OmgClimateDataProcessorController::printVectorAndResult(QVector<float> theVector, float theResult)
{
...
/** Return a nice summary about this OmgClimateDataProcessorController object */
