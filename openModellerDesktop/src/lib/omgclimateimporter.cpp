...
OmgClimateImporter::OmgClimateImporter() : QObject()
{}

OmgClimateImporter::~OmgClimateImporter()
{}

QStringList OmgClimateImporter::import(QString theInputFile, QString theOutputDir, QString theFileType)
{
  ...
  OmgClimateFileReader myReader;
  if (theFileType=="CRU")
  {
    myReader.initialise(theInputFile,OmgClimateFileReader::CRU_CL1_MONTHLY);
  ...
  {
    myReader.initialise(theInputFile,OmgClimateFileReader::HADLEY_SRES_MEAN);
  ...
  OmgMeridianSwitcher mySwitcher;
...
void OmgClimateImporter::propogateError (QString theError)
{
  //just pass it on up the food chain
  emit error(theError);
}

void OmgClimateImporter::propogateMessage (QString theMessage)
{
  //just pass it on up the food chain
